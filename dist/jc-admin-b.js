...
function FichaMedica({ T, patient, updatePatient, removePatient, onBack, onAgendar, initialTab }) {
  ...
  const TABS = [["fichaclinica", "Ficha Cl\xEDnica"], ["mapa", "Mapa facial y antropometr\xEDa"], ["procedimientos", "Procedimientos"], ["imagenes", "Im\xE1genes"], ..._newFeat ? [["examenes", "Ex\xE1menes"]] : [], ["consent", "Consentimientos"], ["receta", "Receta / Indicaciones"], ..._newFeat ? [["presupuesto", "Presupuesto"]] : [], ["facturacion", "Atenciones"], ["campana", "Campa\xF1a"], ["notas", "Notas"], ["resumen", "Resumen IA"], ["auditoria", "Auditor\xEDa IA"]];
...
function FichaMedilinkPanel({ T, patient, onAgendar }) {
  ...
  const rutN = (r) => ("" + (r || "")).replace(/[^0-9kK]/g, "").toLowerCase();
  const mine = appts.filter((a) => a.status !== "anulada" && (a.patId === patient.id || (a.name || "").toLowerCase().trim() === (patient.name || "").toLowerCase().trim() || rutN(a.rut) && rutN(a.rut) === rutN(patient.rut)));
  ...
  const g = (...ks) => {
    for (const k of ks) {
      if (patient[k] != null && patient[k] !== "") return patient[k];
      if (cl[k] != null && cl[k] !== "") return cl[k];
    }
    return null;
  };
  ...
  const vitals = [["Edad", patient.age ? patient.age + " a\xF1os" : "\u2014"], ["Peso", peso ? peso + " kg" : "\u2014"], ["Talla", talla ? talla + " cm" : "\u2014"], ["P. Arterial", pa || "\u2014"], ["Temp", temp ? temp + "\xB0" : "\u2014"], ["Sat O\u2082", sat ? sat + "%" : "\u2014"], ["Frec. C.", fc || "\u2014"], ["IMC", imc ? imc.toFixed(1) : "\u2014"]];
  const preex = (() => {
    const out = [];
    const al = g("alergias", "allergies");
    if (al) out.push(["Alergias", al]);
    const ant = g("antecedentes", "preexistencias", "condiciones");
    if (ant) out.push(["Antecedentes", ant]);
    const med = g("medicamentos", "medicacion");
    if (med) out.push(["Medicaci\xF3n", med]);
    return out;
  })();
  const estadoCol = (a) => window.jcmApptState ? window.jcmApptState(a, T).color : T.accent;
  const wd = ["Domingo", "Lunes", "Martes", "Mi\xE9rcoles", "Jueves", "Viernes", "S\xE1bado"];
  const mm = ["ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"];
  const fechaTxt = (f) => {
    try {
      const d = /* @__PURE__ */ new Date(f + "T00:00:00");
      return wd[d.getDay()] + " " + d.getDate() + " " + mm[d.getMonth()] + " " + d.getFullYear();
    } catch (e) {
      return f;
    }
  };
  const atCard = (a) => /* @__PURE__ */ React.createElement("div", { key: a.id, style: { display: "flex", gap: 12, alignItems: "center", background: T.bg, border: "1px solid " + T.line, borderLeft: "4px solid " + estadoCol(a), borderRadius: 8, padding: "10px 12px" } }, /* @__PURE__ */ React.createElement("div", { style: { flexShrink: 0, textAlign: "center", minWidth: 54 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.serif, fontSize: 18, color: T.text, lineHeight: 1 } }, (a.fecha || "").slice(8) || "\u2014"), /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 9.5, color: T.textMute } }, a.time || "")), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 13, fontWeight: 600, color: T.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, a.proc || "Atenci\xF3n"), /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 10.5, color: T.textMute } }, [a.prof && "Prof.: " + a.prof, a.sucursal].filter(Boolean).join(" \xB7 ") || "Sin profesional")), /* @__PURE__ */ React.createElement("span", { style: { fontFamily: T.sans, fontSize: 9.5, letterSpacing: ".06em", textTransform: "uppercase", color: estadoCol(a), border: "1px solid " + estadoCol(a) + "55", borderRadius: 999, padding: "3px 8px", flexShrink: 0 } }, a.status || "pendiente"));
  const grupo = (titulo, arr, color) => /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", color, marginBottom: 8 } }, arr.length, " ", titulo), arr.length ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 7 } }, arr.slice(0, 6).map(atCard)) : /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 11.5, color: T.textFaint } }, "\u2014"));
  return /* @__PURE__ */ React.createElement("div", { style: { background: T.surface, border: "1px solid " + T.line, borderRadius: 14, padding: "14px 16px", margin: "16px 0 4px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: open ? 14 : 0 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 9 } }, /* @__PURE__ */ React.createElement("span", { style: { fontFamily: T.sans, fontSize: 10, letterSpacing: ".16em", textTransform: "uppercase", color: T.accent, fontWeight: 600 } }, "Resumen del paciente"), /* @__PURE__ */ React.createElement("span", { style: { fontFamily: T.sans, fontSize: 9.5, color: T.textFaint, border: "1px solid " + T.line, borderRadius: 999, padding: "2px 8px" } }, "Vista Medilink \xB7 preview")), /* @__PURE__ */ React.createElement("button", { onClick: () => setOpen((o) => !o), style: { background: "none", border: "1px solid " + T.line, borderRadius: 8, padding: "5px 10px", cursor: "pointer", color: T.textMute, fontFamily: T.sans, fontSize: 11 } }, open ? "Ocultar" : "Mostrar")), open && /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "minmax(260px, 340px) 1fr", gap: 16, alignItems: "start" } }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", color: T.accent, marginBottom: 8 } }, "Signos vitales"), /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 } }, vitals.map(([l, v]) => /* @__PURE__ */ React.createElement("div", { key: l, style: { background: T.bg, border: "1px solid " + T.line, borderRadius: 8, padding: "9px 11px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 8.5, letterSpacing: ".1em", textTransform: "uppercase", color: T.textMute } }, l), /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.serif, fontSize: 17, color: T.text, marginTop: 2 } }, v)))), /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 10, letterSpacing: ".14em", textTransform: "uppercase", color: T.accent, marginBottom: 8 } }, "Preexistencias y medicamentos"), preex.length ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } }, preex.map(([k, v]) => /* @__PURE__ */ React.createElement("div", { key: k, style: { background: T.bg, border: "1px solid " + T.line, borderRadius: 8, padding: "9px 11px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 9, letterSpacing: ".1em", textTransform: "uppercase", color: T.textMute } }, k), /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 12.5, color: T.text, marginTop: 2, lineHeight: 1.4 } }, v)))) : /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 11.5, color: T.textFaint } }, "Sin preexistencias registradas. Compl\xE9talas en la Ficha Cl\xEDnica.")), /* @__PURE__ */ React.createElement("div", null, grupo("pr\xF3ximas atenciones", proximas, T.accent), grupo("atenci\xF3n hoy", hoy, "#1F8A5B"), grupo("atenciones pasadas", pasadas, T.textMute), mine.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { textAlign: "center", padding: "10px 0" } }, /* @__PURE__ */ React.createElement(AdBtn, { T, small: true, primary: true, onClick: () => onAgendar && onAgendar() }, "Agendar primera cita")))));
}
...
function ExamenesTab({ T, patient }) {
  ...
  let ordenes = [];
  try {
    ordenes = (window.DB && window.DB.get("lab_orders") || []).filter((o) => o.patId === patient.id);
  } catch (e) {
  }
  function persist(n) {
    setFiles(n);
...
function PresupuestoTab({ T, patient, updatePatient }) {
  ...
  function docInner() {
    ...
    const inner = jcmMasthead(b) + "<div class='titleblock'><div><div class='eyebrow'>Cotizaci\xF3n</div><h1 class='doc-title'>Presupuesto <span class='it'>de tratamiento</span></h1></div><div class='folio'><span class='k'>Fecha</span><span class='v vbig'>" + e(dot) + "</span></div></div>" + jcmPband(patient, [["RUT", patient.rut], ["Vigencia", (parseInt(validez, 10) || 15) + " d\xEDas"]]) + `<div class='body'><div class='section' style='margin-top:20px'><div class='section-head'><span class='sh-label'>Detalle</span><span class='sh-rule'></span></div><table style="width:100%;border-collapse:collapse;font-family:'Jost',sans-serif;font-size:13px;color:#121A26"><thead><tr><th style='text-align:left;padding:6px 4px;border-bottom:2px solid #121A26'>Tratamiento</th><th style='text-align:center;padding:6px 4px;border-bottom:2px solid #121A26'>Cant.</th><th style='text-align:right;padding:6px 4px;border-bottom:2px solid #121A26'>Valor</th><th style='text-align:right;padding:6px 4px;border-bottom:2px solid #121A26'>Total</th></tr></thead><tbody>` + rows + `</tbody></table><div style="margin-top:14px;text-align:right;font-family:'Jost',sans-serif;font-size:13px;color:#121A26">Subtotal: ` + fmt(subtotal) + (desc ? "<br>Descuento: \u2212 " + fmt(desc) : "") + "<br><span style='font-size:20px;font-weight:600'>Total: " + fmt(total) + "</span></div></div>" + (nota.trim() ? "<div class='section'><div class='section-head'><span class='sh-label'>Notas</span><span class='sh-rule'></span></div><div class='textbox'>" + e(nota).replace(/\n/g, "<br>") + "</div></div>" : "") + "</div>" + jcmSignFoot(b, b.proName, "Presupuesto", patient.name || "", hoy, null);
    return { title: "Presupuesto \xB7 " + e(patient.name || ""), inner, b };
  }
  function imprimir(save) {
    if (!items.length) {
  ...
  function waLink() {
    ...
    items.filter((i) => (i.name || "").trim()).forEach((i) => L.push("\u2022 " + i.name + (i.qty > 1 ? " x" + i.qty : "") + " \u2014 " + fmt((i.price || 0) * (i.qty || 1))));
    if (desc) L.push("Descuento: \u2212 " + fmt(desc));
    L.push("*Total: " + fmt(total) + "*", "V\xE1lido por " + (parseInt(validez, 10) || 15) + " d\xEDas.");
  ...
  const FACE_POS = { "frente": [100, 46], "frontal": [100, 46], "entrecejo": [100, 74], "glabela": [100, 74], "patas de gallo": [150, 88], "ojeras": [124, 100], "nariz": [100, 108], "dorso nasal": [100, 108], "p\xF3mulos": [140, 120], "pomulos": [140, 120], "surcos nasogenianos": [122, 138], "c\xF3digo de barras": [100, 150], "labios": [100, 168], "ment\xF3n": [100, 192], "menton": [100, 192], "l\xEDnea mandibular": [58, 172], "mand\xEDbula": [58, 172], "cuello": [100, 224] };
  function zonaDe(nombre) {
    const n = (nombre || "").toLowerCase();
    for (const k in FACE_POS) if (n.indexOf(k) >= 0) return { k, p: FACE_POS[k] };
    return null;
  }
  function esquemaFacial() {
    ...
    const face = "<svg viewBox='0 0 200 250' width='230' height='288' xmlns='http://www.w3.org/2000/svg'><ellipse cx='100' cy='120' rx='62' ry='84' fill='#F7EFE9' stroke='#D8C7BB' stroke-width='1.5'/><path d='M66 96q30 -18 68 0' fill='none' stroke='#C9B4A6' stroke-width='1.5'/><path d='M92 100 L92 128 Q100 134 108 128' fill='none' stroke='#C9B4A6' stroke-width='1.5'/><path d='M86 154 q14 10 28 0' fill='none' stroke='#C9B4A6' stroke-width='1.5'/><circle cx='80' cy='108' r='3' fill='#7C6A5E'/><circle cx='120' cy='108' r='3' fill='#7C6A5E'/>" + dots + "</svg>";
