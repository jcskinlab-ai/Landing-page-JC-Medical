...
function ReportesOverlay({ T, appts, onBack }) {
  ...
  monthAppts.forEach((a) => {
    ...
    if (a.status === "atendida" || a.attended) porProc[k].real++;
    else porProc[k].pend++;
  });
  const topProc = Object.keys(porProc).map((k) => ({ name: k, pend: porProc[k].pend, real: porProc[k].real, n: porProc[k].pend + porProc[k].real })).sort((a, b) => b.n - a.n).slice(0, 5);
