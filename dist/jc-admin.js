...
function Agenda({ T, appts, patients, addAppt, addPatient, updateAppt, removeAppt, onOpenPatient, onSyncWeb, initialApptId, onConsumeApptId }) {
  ...
  const viewToggleNode = (() => {
    const seg = (k, l) => {
      const on = view === k;
      return /* @__PURE__ */ React.createElement("button", { key: k, onClick: () => setView(k), style: { padding: "7px 16px", borderRadius: luxF ? DS.r.ctl || 9 : 8, border: "none", cursor: "pointer", background: on ? luxF ? T.dark ? "rgba(255,255,255,.14)" : "rgba(255,255,255,.92)" : T.accent : "transparent", color: on ? luxF ? T.text : T.onAccent || "#fff" : T.textMute, fontFamily: T.sans, fontSize: 12.5, fontWeight: on ? 600 : 500, whiteSpace: "nowrap", boxShadow: on && luxF ? "0 1px 3px rgba(0,0,0,.18)" : "none", transition: "background .18s, color .18s" } }, l);
    };
    const wrap = luxF ? { display: "inline-flex", gap: 3, background: T.dark ? "rgba(255,255,255,.05)" : "rgba(255,255,255,.45)", border: "1px solid " + T.line, borderRadius: DS.r.seg || 12, padding: 3, ...daySmallBlur } : { display: "inline-flex", gap: 3, background: T.surface, border: "1px solid " + T.line, borderRadius: 10, padding: 3 };
    return /* @__PURE__ */ React.createElement("div", { style: wrap }, seg("dia", "D\xEDa"), seg("semana", "Semana"), seg("mes", "Mes"));
  })();
