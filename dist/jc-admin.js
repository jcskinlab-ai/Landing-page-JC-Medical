...
function Agenda({ T, appts, patients, addAppt, addPatient, updateAppt, removeAppt, onOpenPatient, onSyncWeb }) {
  ...
  return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 14, marginBottom: 18, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("div", { style: { width: 46, height: 46, borderRadius: 12, background: T.accent + "18", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 } }, /* @__PURE__ */ React.createElement("svg", { width: "22", height: "22", viewBox: "0 0 24 24", fill: "none", stroke: T.accent, strokeWidth: "1.7", strokeLinecap: "round", strokeLinejoin: "round" }, /* @__PURE__ */ React.createElement("rect", { x: "3", y: "4", width: "18", height: "17", rx: "2" }), /* @__PURE__ */ React.createElement("path", { d: "M3 9h18M8 2v4M16 2v4" }))), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 200 } }, /* @__PURE__ */ React.createElement("h1", { style: { fontFamily: T.serif, fontWeight: 300, fontSize: 28, letterSpacing: "-.02em", color: T.text, margin: 0 } }, "Reservas y Citas"), /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 12.5, color: T.textMute, marginTop: 2 } }, "Gestiona la agenda de la cl\xEDnica, confirma asistencias y asigna puntos.")), /* @__PURE__ */ React.createElement("div", { style: { display: "inline-flex", gap: 4, background: T.surface, border: "1px solid " + T.line, borderRadius: 9, padding: 4 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setView("dia"), title: "Vista lista / d\xEDa", style: { display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 32, borderRadius: 7, cursor: "pointer", border: "none", background: view === "dia" ? T.accent : "transparent", color: view === "dia" ? T.onAccent || "#fff" : T.textMute } }, /* @__PURE__ */ React.createElement("svg", { width: "17", height: "17", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.8", strokeLinecap: "round" }, /* @__PURE__ */ React.createElement("path", { d: "M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" }))), /* @__PURE__ */ React.createElement("button", { onClick: () => setView("semana"), title: "Vista calendario / semana", style: { display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 32, borderRadius: 7, cursor: "pointer", border: "none", background: view === "semana" ? T.accent : "transparent", color: view === "semana" ? T.onAccent || "#fff" : T.textMute } }, /* @__PURE__ */ React.createElement("svg", { width: "17", height: "17", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "1.8", strokeLinecap: "round", strokeLinejoin: "round" }, /* @__PURE__ */ React.createElement("rect", { x: "3", y: "4", width: "18", height: "17", rx: "2" }), /* @__PURE__ */ React.createElement("path", { d: "M3 9h18M8 2v4M16 2v4" })))), onSyncWeb && /* @__PURE__ */ React.createElement(AdBtn, { T, onClick: traerWeb }, webBusy ? "Trayendo\u2026" : "\u21BB Traer reservas web"), /* @__PURE__ */ React.createElement(AdBtn, { T, primary: true, onClick: () => setNueva({ time: "10:00", day: view === "dia" ? day : 0 }) }, "+ Nueva Cita")), (() => {
    ...
    const upcoming = todayAppts.filter((a) => {
      const [h, mi] = (a.time || "0:0").split(":");
      return +h * 60 + +mi >= nowM;
    });
    const cleanPh = (s) => (s || "").replace(/\D/g, "");
    const findPat = (a) => {
      const ap = cleanPh(a.phone);
      if (ap.length >= 8) {
        const m = (patients || []).find((x) => {
          const xp = cleanPh(x.phone);
          return xp.length >= 8 && xp.slice(-8) === ap.slice(-8);
        });
        if (m) return m;
      }
      const an = (a.name || "").toLowerCase().trim();
      return (patients || []).find((x) => {
        const xn = (x.name || "").toLowerCase();
        return xn === an || an.length >= 4 && xn.startsWith(an.split(" ")[0]);
      });
    };
    const sinC = todayAppts.filter((a) => {
      const p = findPat(a);
      return p && !p.consent;
    }).length;
    return /* @__PURE__ */ React.createElement("div", { style: { background: T.surface, border: "1px solid " + T.line, borderRadius: 12, padding: "14px 16px", marginBottom: 16 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", color: T.accent } }, "Hoy \xB7 ", todayAppts.length, " cita", todayAppts.length !== 1 ? "s" : ""), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, fontFamily: T.sans, fontSize: 11 } }, upcoming.length > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: T.textMute } }, upcoming.length, " por venir"), sinC > 0 && /* @__PURE__ */ React.createElement("span", { style: { color: "#C0285A", fontWeight: 600 } }, "\u26A0 ", sinC, " sin consentimiento"))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 7 } }, todayAppts.map((a) => {
      const p = findPat(a);
      const hasCons = !p || p.consent;
      const [h, mi] = (a.time || "0:0").split(":");
      const aMins = +h * 60 + +mi;
      const past = aMins < nowM;
      const attended = a.attended || a.status === "attended";
      const noShow = a.status === "no_asistio";
      const col = noShow ? "#C0285A" : attended ? "#1F8A5B" : past ? T.textMute : T.text;
      return /* @__PURE__ */ React.createElement("div", { key: a.id, style: { display: "flex", alignItems: "center", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 12, fontWeight: 600, color: col, width: 44, flexShrink: 0 } }, a.time), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 12.5, color: col, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, a.name || "Sin nombre"), a.proc && /* @__PURE__ */ React.createElement("div", { style: { fontFamily: T.sans, fontSize: 10.5, color: T.textMute, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, a.proc, a.nota ? " \xB7 " + a.nota : "")), !hasCons && /* @__PURE__ */ React.createElement("span", { title: "Sin consentimiento", style: { fontSize: 13, flexShrink: 0 } }, "\u26A0\uFE0F"), noShow && /* @__PURE__ */ React.createElement("span", { style: { fontFamily: T.sans, fontSize: 10, color: "#C0285A", flexShrink: 0 } }, "No asisti\xF3"), attended && !noShow && /* @__PURE__ */ React.createElement("span", { style: { fontFamily: T.sans, fontSize: 10, color: "#1F8A5B", flexShrink: 0 } }, "\u2713"));
    })));
  })(), view === "semana" ? /* @__PURE__ */ React.createElement(SemanaGrid, { T, week, appts, onNew: (off, time) => setNueva({ time, day: off, fromSlot: true }), onEdit: setEdit, updateAppt, removeAppt, onDay: (off) => {
    setDay(off);
    setView("dia");
  }, onVerFicha: (appt) => {
...
function NewCitaModal({ T, patients, addPatient, time, day, onClose, onSave, prefill, appts, onOpenPatient }) {
  ...
  if (step === 2) {
    ...
    return /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { style: { marginTop: 14, paddingTop: 14, borderTop: "1px solid " + T.line } }, /* @__PURE__ */ React.createElement("span", { style: lbl }, "Nota interna ", /* @__PURE__ */ React.createElement("span", { style: { color: T.textMute, textTransform: "none", letterSpacing: 0, fontWeight: 400 } }, "\xB7 solo visible para el equipo")), /* @__PURE__ */ React.createElement("textarea", { value: notaCita, onChange: (e) => setNotaCita(e.target.value), placeholder: "Ej: primera vez, viene con acompa\xF1ante, sensible al dolor\u2026", rows: 2, style: { width: "100%", padding: "10px 13px", borderRadius: 6, border: "1px solid " + T.line, background: T.surface, color: T.text, fontFamily: T.sans, fontSize: 12.5, outline: "none", resize: "none", lineHeight: 1.5 } }))
    );
  }
  return /* @__PURE__ */ React.createElement(
    AdModal,
