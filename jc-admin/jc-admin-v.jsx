...
function DashboardView({ T, D, A, appts, patients, go }) {
  ...
  const funnel = (function () {
    ...
    // Gasto/leads de Meta: el dueño los carga (config). No usamos las campañas demo de CADMIN como reales.
    let spend = 0, leads = 0;
    try { const cfg = (window.DB && DB.get("config")) || {}; spend = +cfg.meta_spend_mes || 0; leads = +cfg.meta_leads_mes || 0; } catch (e) {}
    const demo = !spend; // sin gasto de Meta configurado → muestra el ejemplo ilustrativo (etiquetado)
  ...
  function FunnelBlock() {
    ...
    const Stat = ({ label, value, big, accent }) => (
      <div style={{ flex: 1, background: accent ? (green + "12") : T.surface2, border: "1px solid " + (accent ? green + "44" : T.line), borderRadius: 12, padding: "13px 15px" }}>
        <div style={{ fontFamily: T.sans, fontSize: 9.5, letterSpacing: ".12em", textTransform: "uppercase", color: accent ? green : T.textMute }}>{label}</div>
        <div style={{ fontFamily: T.serif, fontSize: big ? 30 : 19, color: accent ? green : T.text, lineHeight: 1.1, marginTop: 4 }}>{value}</div>
      </div>
    );
    return (
      <div style={{ background: T.surface, border: "1px solid " + T.line, borderRadius: 14, padding: "16px 18px 18px", marginBottom: 16, boxShadow: "0 14px 40px -30px rgba(0,0,0,.4)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
          ...
          {funnel.demo
            ? <span style={{ fontFamily: T.sans, fontSize: 10, color: T.textFaint }}>Datos de ejemplo — se llenan con Meta Ads + Caja</span>
            : <span style={{ fontFamily: T.sans, fontSize: 10, color: T.textFaint }}>Datos reales de tu mes</span>}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 18, alignItems: "start" }}>
                ...
                <div key={st.k} style={{ marginBottom: 11 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 5 }}>
                    ...
                    <span style={{ fontFamily: T.sans, fontSize: 14, fontWeight: 600, color: T.text }}>
                      {st.n}{conv != null && <span style={{ fontSize: 10.5, fontWeight: 400, color: T.textMute, marginLeft: 7 }}>{conv}%</span>}
                    </span>
                  </div>
          ...
          {/* Columna derecha: resultado + ROAS */}
          <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
            <Stat label="Facturaste este mes" value={fmt(funnel.ingresos)} />
            <div style={{ background: green + "12", border: "1px solid " + green + "44", borderRadius: 12, padding: "16px 16px" }}>
              ...
              <div style={{ fontFamily: T.sans, fontSize: 11, color: T.textMute, marginTop: 6, lineHeight: 1.5 }}>
                Invertiste {fmt(funnel.spend)} y facturaste {fmt(funnel.ingresos)}.
              </div>
            </div>
          </div>
        </div>
